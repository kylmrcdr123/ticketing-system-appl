...
public class TicketController {
    ...
    public ResponseEntity<List<Ticket>> getTicketsForUser(@PathVariable String userId) {
        ...
        } else if (userId.length() == 8 && userId.matches("\\d+")) { // Check if ID is 8 digits for employee
            tickets = ticketService.getTicketsByEmployeeNumber(userId);
        } else {
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }

        // Ensure the ticket contains misStaff data
        for (Ticket ticket : tickets) {
            if (ticket.getMisStaff() != null) {
                System.out.println("MIS Staff ID: " + ticket.getMisStaff().getMisStaffNumber());
            }
        }
